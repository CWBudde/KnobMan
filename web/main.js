...
function detachedPreviewRenderFrames() {
  return Math.max(1, parseInt(document.getElementById('prefFrames').value, 10) || 1);
}

function detachedPreviewDelayMs() {
  const duration = Math.max(1, parseInt(document.getElementById('prefDuration').value, 10) || 100);
...
function setDetachedPreviewPlaying(playing) {
  ...
  if (detachedPreviewPlaying) {
    detachedPreviewTimer = setInterval(detachedPreviewTick, detachedPreviewDelayMs());
  }
  const toggle = detachedPreviewWindow.document.getElementById('detachedPreviewToggle');
  if (toggle) {
...
function refreshDetachedPreviewNow() {
  ...
  detachedPreviewFrame = clampInt(currentFrame, 0, detachedPreviewRenderFrames() - 1);
  renderDetachedPreviewFrame(detachedPreviewFrame);
}

function openDetachedPreviewWindow() {
  if (isDetachedPreviewOpen()) return;
  const win = window.open('', 'knobmanDetachedPreview', 'popup=yes,width=520,height=420');
  if (!win) {
...
function getLayerPreviewCached(layerIndex, frame) {
  ...
  const raw = window.knobman_getLayerPreview(layerIndex, frame, LAYER_PREVIEW_SIZE);
...
function normalizeCurvePoints(points) {
  ...
  const unique = [];
  out.forEach(p => {
    if (unique.length === 0 || unique[unique.length - 1].t !== p.t) unique.push(p);
  });
  ...
  if (unique[unique.length - 1].t !== 100) unique.push({ t: 100, l: unique[unique.length - 1].l });
...
function readSelectedCurve() {
  const raw = window.knobman_getCurve ? window.knobman_getCurve(selectedCurve) : null;
  ...
  return {
    ...
    stepReso: clampInt(Number(raw && raw.stepReso != null ? raw.stepReso : 0), 0, 64)
  };
}
...
function drawCurveEditor() {
  ...
  const evalLv = clampFloat(Number(window.knobman_evalCurve ? window.knobman_evalCurve(selectedCurve, ratio) : 0), 0, 100);
...
function deleteSelectedCurvePoint() {
  const state = readSelectedCurve();
  if (curveSelectedPoint <= 0 || curveSelectedPoint >= state.points.length - 1) return;
...
function parseKnobShapeToSvgCommands(path) {
  ...
  const knotText = chunks[0].split(':').map(c => c.trim()).filter(Boolean);
  const knots = [];
  knotText.forEach(ch => {
    const vals = ch.split(',').map(v => Number(v.trim()));
    if (vals.length !== 6 || vals.some(v => !Number.isFinite(v))) return;
    knots.push({ inX: vals[0], inY: vals[1], pX: vals[2], pY: vals[3], outX: vals[4], outY: vals[5] });
  });
  ...
  for (let i = 1; i < knots.length; i++) {
    ...
    out.push({
      cmd: 'C',
      values: [
        toPct(prev.outX), toPct(prev.outY),
        toPct(cur.inX), toPct(cur.inY),
        toPct(cur.pX), toPct(cur.pY)
      ]
    });
  }
  ...
  out.push({
    cmd: 'C',
    values: [
      toPct(last.outX), toPct(last.outY),
      toPct(first.inX), toPct(first.inY),
      toPct(first.pX), toPct(first.pY)
    ]
  });
  out.push({ cmd: 'Z', values: [] });
  return out;
...
function serializeShapeCommands(commands) {
  ...
  (commands || []).forEach(c => {
    ...
    for (let i = 0; i < c.values.length; i += 2) {
      parts.push(formatShapeNumber(c.values[i]), formatShapeNumber(c.values[i + 1]));
    }
  });
  return parts.join(' ');
}
...
function shapeHandles(commands) {
  ...
  (commands || []).forEach((c, idx) => {
    if (c.cmd === 'M' || c.cmd === 'L') {
      handles.push({ cmdIndex: idx, valueIndex: 0, role: 'anchor', x: c.values[0], y: c.values[1] });
    } else if (c.cmd === 'Q') {
      handles.push({ cmdIndex: idx, valueIndex: 0, role: 'control', x: c.values[0], y: c.values[1] });
      handles.push({ cmdIndex: idx, valueIndex: 2, role: 'anchor', x: c.values[2], y: c.values[3] });
    } else if (c.cmd === 'C') {
      handles.push({ cmdIndex: idx, valueIndex: 0, role: 'control', x: c.values[0], y: c.values[1] });
      handles.push({ cmdIndex: idx, valueIndex: 2, role: 'control', x: c.values[2], y: c.values[3] });
      handles.push({ cmdIndex: idx, valueIndex: 4, role: 'anchor', x: c.values[4], y: c.values[5] });
    }
  });
  return handles;
}
...
function drawShapeOverlay() {
  ...
  handles.forEach(h => {
    const p = shapeToCanvas(h.x, h.y, m);
    const selected = Boolean(shapeSelectedHandle &&
...
function refreshShapeEditor() {
  ...
  if (!active) {
    ...
    shapeDragHandle = null;
    shapeSelectedHandle = null;
    if (hint) hint.textContent = 'Select a layer with Primitive = Shape to edit its path.';
    if (shapeCtx && shapeCanvas) shapeCtx.clearRect(0, 0, shapeCanvas.width, shapeCanvas.height);
    return;
  }
  if (hint) hint.textContent = 'Click to add points. Drag handles to edit. Right click or Delete removes selected segment.';
...
async function ensureBuiltinTextures() {
  ...
  builtinTextureLoadPromise = (async () => {
    const existing = window.knobman_getTextureList ? (window.knobman_getTextureList() || []) : [];
    const byName = new Set(existing.map(t => String(t.name || '').toLowerCase()));
...
function wireControls() {
  ...
  document.getElementById('prefBgAlpha').addEventListener('input', onPrefsChange);
  ...
  document.getElementById('btnPreviewWin').addEventListener('click', toggleDetachedPreviewWindow);
...
function syncPrefsFromGo() {
  ...
  if (p.bgAlpha != null) document.getElementById('prefBgAlpha').value = p.bgAlpha;
...
function appendTexturePanel(content, primType) {
  ...
  const textures = window.knobman_getTextureList ? (window.knobman_getTextureList() || []) : [];
  const selectedTexture = window.knobman_getParam(selectedLayer, 'textureFile') ?? 0;
  ...
  textures.forEach(tex => {
    ...
    card.type = 'button';
    card.className = 'texture-card' + (tex.index === selectedTexture ? ' active' : '');
    card.title = tex.name;
    ...
    const arr = window.knobman_getTextureData ? window.knobman_getTextureData(tex.index) : null;
    if (arr && arr.length) {
      const img = document.createElement('img');
